...
/*
  maxIdx = 方式ごとの到達シェード上限(SHADESのindex)。
  - オフィス/ホーム: 過酸化物による漂白のため B1(idx6) まで
  - セルフ: 非医療・過酸化物不使用のため「歯を漂白」できない。
    表面の着色(ステイン)除去で本来の歯の色に戻るイメージとして A1(idx4) を上限とする。
    ※これを超える表示は優良誤認(景表法)のリスクがあるため変更しないこと
*/
const METHODS = [
...
export default function WhiteningSimulator() {
  ...
  const [startShade, setStartShade] = useState(2); // 今の歯の色: 1=A3.5 / 2=A3(日本人平均) / 3=A2
  const [simIntent, setSimIntent] = useState(null); // CTA経由の来訪時に { method, shade } を保持し店舗リストに引き継ぐ
  ...
  // 画像の白色化も方式上限でキャップし、シェード表示と画像が常に一致するようにする
  // (セルフでB1相当まで画像だけ白くなる、といった乖離を防ぐ)
  const intensity = Math.min(rawIntensity, (maxIdx - startShade) / 7);
  // 到達目安の上限はB1(idx6)。BLはシェードガイドの表示のみで、シミュレーション上は到達しない
  const shadeIdx = Math.min(maxIdx, startShade + Math.round(intensity * 7));
  const atSelfCeiling = m.id === "self" && rawIntensity > intensity + 0.001;
      ...
      {screen === "home" && (
        <main>
          ...
          <div id="hm-clinics" className="hm-container" style={{ paddingTop: 26, paddingBottom: 40 }}>
            ...
            {simIntent && (
              <div style={{ fontSize: 11.5, lineHeight: 1.7, marginBottom: 12, padding: "10px 14px", background: C.champagne, border: `1px solid ${C.line}`, borderRadius: 12, color: C.ink }}>
            ...
            <div className="hm-clinics">
              {(simIntent
                ...
                const matched = simIntent && c.methods?.includes(simIntent.method);
                const mismatched = simIntent && !c.methods?.includes(simIntent.method);
                return (
                <div key={c.name} style={{ background: C.card, borderRadius: 18, padding: 18, border: matched ? `2px solid ${C.gold}` : `1px solid ${C.line}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                    <div>
                      ...
                      {mismatched && (
                        <div style={{ fontSize: 10.5, color: C.sub, marginTop: 5 }}>※シミュレーションで選んだ方式({simIntent.methodLabel})とは異なる方式のお店です</div>
                      )}
                    </div>
        ...
        <main style={{ maxWidth: 560, margin: "0 auto", padding: "0 16px 40px" }}>
          ...
          {imgSrc && (
            <>
              ...
              <div style={{ marginTop: 16, background: C.card, borderRadius: 16, padding: 16, border: `1px solid ${C.line}` }}>
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {[{ idx: 1, label: "A3.5", desc: "やや濃いめ" }, { idx: 2, label: "A3", desc: "平均的" }, { idx: 3, label: "A2", desc: "明るめ" }].map((s) => (
                      <button key={s.idx} onClick={() => setStartShade(s.idx)}
                        style={{
                          flex: 1, borderRadius: 10, padding: "8px 4px",
                          border: startShade === s.idx ? `2px solid ${C.gold}` : `1.5px solid ${C.line}`,
                          background: startShade === s.idx ? C.champagne : C.card,
                        }}>
                        <div style={{ width: 18, height: 18, borderRadius: 5, background: SHADES[s.idx].hex, margin: "0 auto 4px", border: `1px solid ${C.line}` }} />
                        <div style={{ fontSize: 11, fontWeight: startShade === s.idx ? 900 : 700 }}>{s.label}</div>
                        <div style={{ fontSize: 9, color: C.sub }}>{s.desc}</div>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ marginTop: 16 }}>
                  ...
                  {atSelfCeiling && (
                    <div style={{ fontSize: 11, lineHeight: 1.7, marginTop: 8, padding: "10px 12px", background: C.champagne, border: `1px solid ${C.line}`, borderRadius: 10, color: C.ink }}>
                      セルフホワイトニングは歯の表面の着色(ステイン)ケアが中心のため、目安は「本来の歯の色」までとしています。これ以上の白さをめざす場合は、歯科医院で受けられる<b>オフィス</b>や<b>ホーム</b>ホワイトニングが選択肢になります。
                      <button onClick={() => { setMethod("office"); track("sim_switch_office_from_self", { sessions }); }}
                        style={{ display: "block", marginTop: 8, background: "none", border: `1.5px solid ${C.gold}`, color: C.goldDark, fontWeight: 900, fontSize: 11, borderRadius: 999, padding: "6px 14px" }}>
                        オフィスの目安に切り替えてみる →
                      </button>
                    </div>
                  )}
              ...
              {/* ---------- CTA ---------- */}
              <button onClick={() => { setSimIntent({ method: m.id, methodLabel: m.label, shade: SHADES[shadeIdx].name }); setScreen("home"); track("sim_to_clinics", { method: m.id, sessions, shade: SHADES[shadeIdx].name }); requestAnimationFrame(() => document.getElementById("hm-clinics")?.scrollIntoView({ behavior: "smooth" })); }}
